...
public class OmokTest3 {
	...
	static void initBoard(char[][] board) {
		...
		// 상하 마감
		for (int x = 0; x < 10; x++)
			board[0][x] = '┬'; // 세팅해준board는 메인에 있는 board도 같이 세팅해준다! 공유하는 것!
	...
	public static void main(String[] args) {
		...
		char[][] board = new char[10][10]; // 문자 100개를 담을 배열 만들기 콜바이 래퍼런스
		int count = 0;
		initBoard(board);

		PlayOmokOnBoard(board, count);
	...
	static void PlayOmokOnBoard(char[][] board, int count) {
		while (true) {
			{
